...
from mydb import SQLAlchemy

db = SQLAlchemy('mysql://root:yawen00@localhost/hunt?charset=utf8')
...
class JobRequest(db.Model):
    ...
    checked              = db.Column(db.Boolean)     # 是否已被用户察看
    ignored              = db.Column(db.Boolean)     # 是否已被用户忽略
    hr_evaluate          = db.Column(db.String(16))  # HR 对用户的评价
    hr_msg               = db.Column(db.String(255)) # HR 附带给用户的信息
    callbackdate         = db.Column(db.DateTime)    # HR 反馈的时间
